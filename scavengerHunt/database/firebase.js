...
function addCategoryToScavengerHuntItem(itemId, categoryName){
	...
	// Sets the update object to be added to the database
	let update = {}
	update['/scavenger_hunt_items/' + itemId + '/category/' + categoryName] = true
	return database.ref().update(update)
}

//  Associates a scavenger hunt item with a map, only the map should have a reference to the items
...
//  Seeds scavenger hunt list items
writeScavengerHuntItem(1, 'Open Market', '15 William St, New York, NY 10005, USA', 40.7052066, -74.0103288999999)
writeScavengerHuntItem(2, 'Le Pain Quotidien', '85 Broad St, New York, NY 10005, USA', 40.7039915, -74.0110917)
...
//  Seeds categories (do we really need these?)
...
//  Adds categories to scavenger hunt items
...
//  Associates maps with their scavenger hunt list items
